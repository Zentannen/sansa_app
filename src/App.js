...
function App() {
  let value = useMediaQuery('prefers-color-scheme: dark')
    ...
    const [accents,setAccents] = useState(localStorage.getItem('accents')===null?{dark:'#55f',message:'#66f',light:'#44f'}:JSON.parse(localStorage.getItem('accents')))
    ...
    useEffect(()=>{
      document.body.style.backgroundColor=theme.theme?'#444':'#fff'
    })
  return (
